...
2985 <- read10xCounts(path.2985hpc.neun, col.names=T)
...
2986 <- read10xCounts(path.2986hpc.neun, col.names=T)
...
2987 <- read10xCounts(path.2987hpc.neun, col.names=T)
...
2988 <- read10xCounts(path.2988hpc.neun, col.names=T)
...
colData(sce.total)$condition<-factor(
  ifelse(
    colData(sce.total)$sample_name %in% c("2985","2986"),
         "control","seizure")
)
...
hdg<-rownames(counts(sce.subset))[1:1000]
