...
pdf("human_hpc_violin_markers_mnn50.pdf", height=6, width=12)
for(i in 1:length(markers.custom)){
  print(
    plotExpression(sce.human.hpc, exprs_values = "logcounts", features=c(markers.custom[[i]]),
...
boxplot(data$transcripts ~ data$cluster,ylab="Transcripts",xlab="Cluster",cex.lab=1,cex.axis=1,las=2,ylim=c(0,5)) +
  title("Transcripts per cluster",cex.main=1.5) +
  dev.off()
